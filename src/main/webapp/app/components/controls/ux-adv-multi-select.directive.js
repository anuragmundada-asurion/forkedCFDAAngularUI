    ...
    function advMultiSelect($parse, $filter, $timeout, util) {
        ...
        function compile(element, attr) {
            ...
            var choicesTemplate = '' +
                ...
                '<ul role="listbox">' +
                '<li role="option" tabindex="0" ng-click="_ctrl.toggle(choice)" trigger-click-on-enter ng-class="{\'selected\': _ctrl.isSelected(choice)}" ng-repeat="choice in {0} | {1} limitToCFDA:_ctrl.itemsPerPage:_ctrl.offset">{{ _ctrl.choiceTitleGetter(choice) }}</li>' +
                '</ul>' +
                '</div>' +
