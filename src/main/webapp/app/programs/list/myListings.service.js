!function() {
    ...
    myApp.service('MyListingsService', ['DTColumnBuilder', 'AuthorizationService', 'PERMISSIONS', '$filter', function(DTColumnBuilder, AuthorizationService, PERMISSIONS, $filter) {
        ...
        this.getActionContent = function(data) {
            ...
            if (!archived) {
                ...
                if (status === 'published' && (AuthorizationService.authorize(PERMISSIONS.CAN_REQUEST_ARCHIVE) || AuthorizationService.authorize(PERMISSIONS.CAN_REQUEST_TITLE_CHANGE) || AuthorizationService.authorize(PERMISSIONS.CAN_REQUEST_AGENCY_CHANGE))) {
                    var actions = '<div class="ui icon top left pointing jqChangeRequest dropdown button">'+
        ...
        this.getRequestActionContent = function(data) {
            return '<a ui-sref="viewRequest({ id: \'' + data + '\' })" class="ui mini primary button" title="Review Program"><span class="fa fa-eye"></span></a>';
        };
